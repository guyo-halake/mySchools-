...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto font-sans text-zinc-900 animate-in fade-in duration-500">
      ...
      {(user.role === 'PARENT' || user.role === 'STUDENT') && <PersonalPortal user={user} />}
    </div>
  );
};
...
const PersonalPortal = ({ user }: any) => {
  ...
  useEffect(() => {
    const fetchPortalData = async () => {
      try {
        ...
        const myChildren = allStudents.filter((s: any) => s.parent_id === user.id || s.id === user.id);
        setChildren(myChildren);

        if (myChildren.length > 0) {
          const mainChild = myChildren[selectedChildIndex];
          const [results, fees, health, discipline, assignments, events, announcements] = await Promise.allSettled([
  ...
  if (!envData.student) return (
    <div className="py-20 text-center space-y-4 max-w-sm mx-auto">
      <ShieldAlert size={48} className="mx-auto text-zinc-300" />
      <h2 className="text-lg font-bold">Verification Pending</h2>
      <p className="text-sm text-zinc-500">Contact the registrar to link your student account.</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
        <div className="text-left space-y-1">
          ...
          <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest italic">
             {student.profile?.full_name} • {student.adm_no}
          </p>
        </div>
      ...
      <div className="pb-32 md:pb-12 px-2 md:px-4">
        {activeTab === 'home' && <HomePortal env={envData} setActiveTab={setActiveTab} />}
        {activeTab === 'learning' && <LearningPortal env={envData} />}
        {activeTab === 'results' && <ResultsPortal env={envData} />}
        {activeTab === 'fees' && <FeesPortal env={envData} isParent={user.role === 'PARENT'} />}
        {activeTab === 'events' && <EventsPortal env={envData} />}
        {activeTab === 'more' && <MorePortal env={envData} isParent={user.role === 'PARENT'} />}
      </div>
...
const HomePortal = ({ env, setActiveTab }: any) => {
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-10">
           <div className="flex items-center gap-6">
              ...
              <div className="text-left space-y-2">
                 <h2 className="text-3xl font-black tracking-tight leading-none uppercase">{student.profile?.full_name}</h2>
                 <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest leading-none">
                    {student.stream?.class?.name} {student.stream?.name} • ADM {student.adm_no}
                 </p>
...
const FeesPortal = ({ env, isParent }: any) => {
  const { fees } = env;
  const balance = (fees || []).reduce((acc: number, f: any) => acc + (f.amount_due - f.amount_paid), 0);
         ...
         <div className="bg-zinc-950 rounded-[3.5rem] p-12 text-white space-y-12 relative overflow-hidden group shadow-2xl">
            <div className="relative z-10 space-y-12">
               ...
               <div className="space-y-6">
                  ...
                  {!isParent && <p className="text-center text-[9px] font-black uppercase text-zinc-500 tracking-widest">Note: Payments are restricted to Parent accounts.</p>}
               </div>
            </div>
...
const MorePortal = ({ env, isParent }: any) => {
         ...
         <section className="lg:col-span-2 bg-blue-600 rounded-[4rem] p-16 text-white relative overflow-hidden group shadow-2xl shadow-blue-500/20">
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-16 text-center md:text-left">
               <div className="space-y-10">
                  ...
                  <button 
                    disabled={!isParent}
                    className="px-12 py-6 bg-white text-blue-600 rounded-[2rem] font-black text-[10px] uppercase tracking-widest shadow-2xl hover:scale-105 active:scale-95 disabled:grayscale disabled:opacity-50 transition-all"
                  >
