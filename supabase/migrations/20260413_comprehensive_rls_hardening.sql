...
DECLARE
  table_name TEXT;
BEGIN
  -- Check RLS enabled on all critical tables
  FOR table_name IN 
    SELECT unnest(ARRAY[
  ...
  LOOP
    IF NOT EXISTS (
      ...
      WHERE table_name = table_name AND row_security = true
    ) THEN
...
RAISE NOTICE 'All user data is now strictly isolated by school_id at database layer';
