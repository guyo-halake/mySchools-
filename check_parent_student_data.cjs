...
async function checkParentStudentData() {
  try {
    ...
    const studentsRes = await pgClient.query('SELECT students.id, students.adm_no, profiles.full_name FROM students JOIN profiles ON students.id = profiles.id WHERE students.parent_id = $1', [parentId]);
